		...
		for z in range(0,progress_len):
			for x in range(0,pWidth_pix*pheight_pix):
				if(z == 0):
					...
					w_buffer.insert(y,temp[x])					
					y = y + 1
		self.EPD_Dis_Part(xDot-xDot/16-4,xDot-xDot/16+7,(yDot-16*progress_len)/2-1,(yDot-16*progress_len)/2+16*progress_len,w_buffer,1)
		#2.Load progress bar
		y =0
		for z in range(0,progress_len):
			for x in range(0,pWidth_pix*pheight_pix):
		...
		for z in range(0,8):
			for x in range(0,len_s):
				if z == 0 :
					...
					return 1
		self.EPD_Dis_Part(yStart,yStart+8,yDot-xStart-16*8+1,yDot-xStart,w_buffer,1)
